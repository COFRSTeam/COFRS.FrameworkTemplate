using COFRS.Common.Template.Models;
using COFRS.Common.Template.ServiceUtilities;
using EnvDTE;
			...
			foreach (var line in lines)
			{
				...
				else if (line.Trim().StartsWith("public", StringComparison.OrdinalIgnoreCase))
				{
					...
					if (match2.Success)
					{
						...
						if (dbType == DBServerType.SQLSERVER)
							dataType = DBHelper.ConvertSqlServerDataType(nativeDataType);
						else if (dbType == DBServerType.MYSQL)
							dataType = DBHelper.ConvertMySqlDataType(nativeDataType);
						else if (dbType == DBServerType.POSTGRESQL)
							dataType = DBHelper.ConvertPostgresqlDataType(nativeDataType);
				...
				using (var command = new NpgsqlCommand(query, connection))
				{
					...
					using (var reader = command.ExecuteReader())
					{
						while (reader.Read())
						{
							...
							var elementName = BaseUtilities.NormalizeClassName(element);
					...
					using (var reader = command.ExecuteReader())
					{
						while (reader.Read())
						{
							...
							if ( dbColumn == null )
                            {
								...
								dbColumn.ColumnName = BaseUtilities.NormalizeClassName(reader.GetString(0));
								classFile.Columns.Add(dbColumn);
							}
